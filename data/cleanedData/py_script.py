...
def detect_column_types(df):
    ...
    text_keywords = ["name", "author", "text", "review", "comment", "description", "category", "type", "class", "label"]

    numeric_cols, text_cols = [], []

    for col in df.columns:
        col_lower = col.lower()
        # Priority: if matches both, treat as numeric first
        if any(k in col_lower for k in text_keywords):
...
# -------------------------------
def normalize_numeric_columns(df, numeric_cols):
...
def clean_csv(file_path):
    ...
    text_cols, numerical_cols = detect_column_types(df)
    df = normalize_text_columns(df, text_cols)
    df = normalize_numeric_columns(df, numerical_cols)
